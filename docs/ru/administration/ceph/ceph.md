...
Основные группы для Ceph кластера, которые требуются для развертывания {var(sys2)}:
...
#### {heading(Просмотр общей статистики RBD-пула)[id=ceph_view_rbd_pool]}
...
В {var(sys3)} поддержка тонких томов выполняется по умолчанию (параметр `thin_provisioning_support` указан в `rbd` драйвере: `./cinder/volume/drivers/rbd.py`).
