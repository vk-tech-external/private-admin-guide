...
1. Подготовьте установочный WIM-файл ОС.
   ...
   Установочный образ может содержать в себе несколько редакций операционной системы. Так как установка проходит в автоматическом режиме, заранее выберите нужную редакцию и экспортируйте её в отдельный wim-файл.
   ...
   1. Выведите список всех версий Windows с помощью выполненной от имени администратора команды:
      ...
      Появится список редакций с указанием `ImageIndex` её номера:
...
Список поддерживаемых образов приведён в [официальной документации](https://docs.openstack.org/diskimage-builder/latest/user_guide/supported_distros.html) утилиты `diskimage-builder`.
