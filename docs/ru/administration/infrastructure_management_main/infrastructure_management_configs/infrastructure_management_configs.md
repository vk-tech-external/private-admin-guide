...
Локальное изменение конфигураций на отдельных узлах (без изменений на деплой-ноде) приведет к их «затиранию» при следующем обновлении или восстановлении сервисов/узлов {ifdef(box, pg)}{var(sys2)}{/ifdef}{ifndef(box, pg)}{var(sys2_go)}{/ifndef} с деплой-ноды.
...
* Установка сервиса с нуля — выполняются дополнительные действия, например создание БД: `bootstrap=true`. Список дополнительных действий может отличаться в зависимости от сервиса.
