...
Для просмотра списка вычислительных узлов через Портал администратора перейдите в раздел **Облачные вычисления** → **Гипервизоры**. Список вычислительных узлов отобразится в таблице.

### {heading(OpenStack CLI)[id=cli_hypervisor_list]}

Просмотр списка вычислительных узлов можно выполнить при помощи OpenStack CLI. Для этого выполните подготовительные операции (подробности приведены в разделе {linkto(../../../usage/interfaces_access#use_openstackcli_admin)[text=%text]}), затем — команды:
...
Для просмотра доступных ресурсов на вычислительном узле в кластере через Портал администратора перейдите в раздел **Облачные вычисления** → **Гипервизоры**:
...
Просмотр доступных ресурсов на вычислительном узле в кластере можно выполнить при помощи OpenStack CLI. Для этого выполните подготовительные операции (подробности приведены в разделе {linkto(../../../usage/interfaces_access#use_openstackcli_admin)[text=%text]}), затем — команду:
...
1. При помощи OpenStack CLI уточните текущий список и статус вычислительных узлов, например, выполнив команду (подробности приведены в разделе {linkto(#cli_hypervisor_list)[text=%text]}):
...
1. Отключите возможность запуска новых ВМ на вычислительном узле, выполнив команду:
...
{ifndef(cer)}
1. Для всех ВМ, запущенных на вычислительном узле, выводимом из кластера, выполните миграцию (подробности приведены в разделе {linkto(../../../usage/iaas_main/iaas_vm_managing#vm_migration)[text=%text]}).
{/ifndef}
...
1. Уточните текущий список и статус запущенного вычислительного узла (подробности — в разделе {linkto(#cli_hypervisor_list)[text=%text]}). Ожидаемые значения в колонке **Status** — `enabled`, в колонке **State** — `up`.

{ifndef(cer)}
1. При необходимости выполните миграцию ВМ для разгрузки запущенных вычислительных узлов на ранее включенный узел (подробнее — в разделе {linkto(../../../usage/iaas_main/iaas_vm_managing#vm_migration)[text=%text]}).
{/ifndef}
...
Однако в {var(sys3)} существует возможность ограничить использование вычислительного узла одним выбранным проектом/тенантом. Закрепление выделенного вычислительного узла за каким-либо проектом означает, что ресурсы данного узла будут доступны только в рамках этого проекта.
...
1. Перейдите в раздел **Облачные вычисления** → **Агрегаторы узлов**.
...
1. Нажмите кнопку **Создание агрегатора узлов**. Дождитесь завершения операции.
1. На странице **Агрегаторы узлов** нажмите на значок слева от поля поиска и выберите вариант **Реальные данные** (по умолчанию показаны **Кешированные данные**).
1. Найдите в списке созданный агрегатор, справа от его названия нажмите на значок **•••** и выберите пункт **Обновить метаданные**.
...
Если необходимо, чтобы ВМ проекта запускались только на определенном вычислительном узле, нужно создать и использовать специальный тип ВМ (flavor) для проекта с параметром `aggregate_instance_extra_specs:filter_tenant_id`, например:
...
{ifndef(cer)}
1. Выполните миграцию ВМ для разгрузки других вычислительных узлов при необходимости (подробнее — в разделе {linkto(../../../usage/iaas_main/iaas_vm_managing#vm_migration)[text=%text]}).
{/ifndef}
...
1. Перейдите на деплой-ноду по SSH (подробнее — в разделе {linkto(../../../usage/interfaces_access#deploy_host_auth)[text=%text]}).
1. В файл `~/inventory/<ENV>/<ENV>.yml`, где `<ENV>` — название инсталляции/окружения, добавьте информацию о новом вычислительном узле в секцию `hosts`:
...
Запустите плейбуки из директории `~/inventory/<ENV>`, выполнив команды:

```console
$ ansible-playbook -i vkcloud.yml -e env=vkcloud ../ansible-openstack/playbooks/zabbix-openstack-deploy.yml -D --limit <NEW_HOST>
$ ansible-playbook -i vkcloud.yml -e env=vkcloud ../ansible-openstack/playbooks/compute-deploy.yml -D -e ansible_user=centos -e admin_user_name=centos --limit <NEW_HOST>
```

где:

* `vkcloud` — название окружения.
* `<NEW_HOST>` — название создаваемого вычислительного узла.

{caption(Пример запуска плейбуков на stage для вычислительного узла kcn002)[align=left;position=above]}
```console
$ ansible-playbook -i stage.yml -e env=stage ../ansible-openstack/playbooks/zabbix-openstack-deploy.yml -D --limit kcn002
$ ansible-playbook -i stage.yml -e env=stage ../ansible-openstack/playbooks/compute-deploy.yml -D -e ansible_user=centos -e admin_user_name=centos --limit kcn002
```
{/caption}
...
1. Перейдите на управляющий узел с поддержкой OpenStack CLI (подробнее — в разделе {linkto(../../../usage/interfaces_access#use_openstackcli_admin)[text=%text]}).
...
где `<NEW_HOST>` — название создаваемого вычислительного узла.
...
```console
# openstack compute service list --host kcn002 --long
```
...
1. Из таблицы выберите любой рабочий хост (например, `kcn003`), нажав на его имя.
...
1. Нажмите кнопку **Add**. Дождитесь изменения индикации надписи `[ZBX]` в столбце **Availability** на зелёный.
...
1. Перейдите на любой управляющий узел по SSH (например, `cpn003`).
...
1. Перейдите на деплой-ноду по SSH (подробнее — в разделе {linkto(../../../usage/interfaces_access#deploy_host_auth)[text=%text]}).
1. В файле `~/inventory/<ENV>/<ENV>.yml` удалите информацию о вычислительном узле из секций `hosts` и `host_vars`.
...
1. Если в очереди есть непрочитанные сообщения, очистите очередь, выполнив команду:

   ```console
   # rabbitmqctl purge_queue <QUEUE_NAME>
   ```

   где `<QUEUE_NAME>` — название очереди из предыдущего шага, в которой содержались сообщения.
...
1. Получите токен авторизации (подробнее — в разделе {linkto(../../../usage/interfaces_access#http_api_get_auth_token)[text=%text]}).
...
1. Проверьте наличие аллоцированных ресурсов для данного вычислительного узла, выполнив команду:
...
1. Если в результате выполнения команды получен пустой объект `"allocations": {}`, удалите resource provider, выполнив команду:
