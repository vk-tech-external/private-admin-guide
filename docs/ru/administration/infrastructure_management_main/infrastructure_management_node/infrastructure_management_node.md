...
Не все управляющие узлы полностью равнозначны. Один из управляющих узлов содержит в себе мастер-ноды кластеров СУБД Tarantool (Dusk), которые используются другими компонентами {var(sys2_go)}. Все остальные управляющие узлы содержат реплики этих кластеров (доступные только для чтения).

<err>

В случае недоступности Главного управляющего узла (например, его выхода из строя или отключения/перезагрузки) становятся недоступны основные интерфейсы управления {var(sys2_go)}: Портал самообслуживания, Портал администратора, API, CLI. При этом работа виртуальных вычислительных ресурсов {var(sys2_go)} (ВМ, сетей, СУБД и кластеров) продолжается в штатном режиме, однако их остановка или запуск новых ресурсов остаются временно недоступны.

После восстановления работоспособности Главного управляющего узла все функции {var(sys2_go)} должны работать в штатном режиме.
...
Для определения того, является ли управляющий узел Главным управляющим узлом, достаточно подключиться к одному из инстансов Tarantool командой `tt connect <ИМЯ_ИНСТАНСА>` и проверить значение параметра `read_only` из `box.cfg` с помощью команды `box.info.ro`.

{caption(Команды для подключения к инстансу Tarantool и проверки запрета записи)[align=left;position=above]}
```console
$ sudo -i
# tt connect breeze_01
# box.info.ro
```
{/caption}
...
1. Переключитесь на пользователя `root`, выполнив команду `sudo -i`.
...
Восстановление БД необходимо выполнить, только когда нет других вариантов.
...
* Файл резервной копии будет скопирован на узел, выбранный в качестве bootstrap-ноды для кластера, и не будет удален.
