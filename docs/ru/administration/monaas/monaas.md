...
Чтобы установить агент мониторинга на ВМ при ее создании, пользователь Портала самообслуживания ставит переключатель **Включить мониторинг** в активное положение. При этом:

1. В сервис templater отправляется запрос.
1. В ответ на запрос приходит скрипт-установщик cloud-init script.
1. Скрипт скачивает и устанавливает пакеты, создает конфигурационные файлы, подставляет в них реквизиты для входа сервисного пользователя.
...
1. При выполнении команды создается сервисный пользователь и приходит скрипт-установщик cloud-init script.
1. Скрипт скачивает и устанавливает пакеты, создает конфигурационные файлы, подставляет в них реквизиты для входа сервисного пользователя.
