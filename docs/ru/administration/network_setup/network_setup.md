# {heading(Настройка сети)[id=network_setup]}
...
Удаление сетевых подключений через Портал администратора:

1. Перейдите в раздел **Сетевая подсистема** → **Порты**.
...
Удалять сети и сетевые подключения можно через Портал самообслуживания. Шаги по выполнению операций приведены в **Руководстве пользователя {var(system)}** в разделе **Сети**.
...
## {heading(Добавление нового сегмента внешних адресов)[id=adding_new_external_address_segment]}

Для добавления подсети адресов во внешнюю сеть создайте новую подсеть в этой внешней сети (подробнее — в разделе {linkto(../../usage/iaas_main/iaas_network_settings#subnet_creating)[text=%text]}).
