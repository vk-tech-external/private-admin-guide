...
![Структурная схема celery-zabbix](./assets/celery_zabbix_str_scheme.png){params[width=70%;printWidth=70%;noBorder=true]}
{/caption}

Сервис мониторинга функционирует на отдельном хосте роли Logging-Monitoring. На этом же хосте развертывается отдельный экземпляр БД MariaDB. Для доступа к данным используется Портал мониторинга (Zabbix).
