...
Портал самообслуживания — графический интерфейс пользователя {var(sys2)}. Портал используется для управления виртуальными ресурсами в рамках одного или нескольких проектов. Каждая учетная запись может иметь доступ к неограниченному количеству проектов. Роли пользователей и их доступ к функциям {var(sys2)} могут отличаться от проекта к проекту.
...
Для входа в Портал самообслуживания используется любая персональная учетная запись пользователя {var(sys2)}.
...
{ifdef(cer)}
Для входа в Портал администратора используется учетная запись {var(sys2)}, для которой были выставлены роли с префиксом `sa_`.
{/ifdef}
...
Для входа в Портал управления доступом используется стандартная учетная запись {var(sys2)}, имеющая соответствующие права доступа.
...
По умолчанию OpenStack CLI развернут на узлах {var(sys2)} и не требует дополнительной настройки. При использовании локального OpenStack CLI может потребоваться установка зависимостей, подробнее — в разделе {linkto(../../usage/interfaces_access#prerequisites_openstack_cli_software)[text=%text]}.
...
Интерфейс командной строки для развертывания, конфигурирования и обновления компонентов {var(sys2)}.
...
Пример использования приведен в разделе {linkto(../../usage/interfaces_access#interface_rest_api)[text=%text]}.
{/ifndef}
