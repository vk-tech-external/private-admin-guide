...
{var(sys1)} представляет собой программный комплекс на базе программного обеспечения с открытым исходным кодом (OpenStack и др.) и программного обеспечения собственной разработки.
...
Список компонентов собственной разработки представлен далее в {linkto(#tab_own_dev_software)[text=таблице %number]}.
...
|Keycloak
|Управление учётными записями пользователей, группами пользователей, интеграция с LDAP-провайдером, SSO
...
|MiniO
|Высокопроизводительное объектное хранилище. Используется как тестовое приложение в SaaS
