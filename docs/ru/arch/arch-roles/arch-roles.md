...
|Администратор Портала управления доступом Keycloak. Имя учетной записи по умолчанию — `admin`. Обладает доступом ко всем учетным записям {var(sys2)} (кроме `superadmin` и тех, что хранятся в службе каталогов LDAP-провайдера) и настройкам их доступа.
...
Служебный пользователь с ролью **Владелец проекта** создается в единственном экземпляре при первоначальной настройке {var(sys2)} в Keystone и представляет собой единственного владельца всех проектов {var(sys2)}. Имя задается при создании. Данная учетная запись редактированию не подлежит.
...
{ifndef(box)}
{caption(Таблица {counter(table)[id=numb_tab_roles]} — Права пользователей Портала самообслуживания)[align=right;position=above;id=tab_roles;number={const(numb_tab_roles)}]}
![Права пользователей Портала самообслуживания](./assets/roles-go.png){params[printWidth=140mm;printHeight=210mm;noBorder=true]}
{/caption}
{/ifndef}
...
![Права пользователей Портала администратора](./assets/sa_roles.png){params[printWidth=140mm;printHeight=210mm;noBorder=true]}
{/caption}
...
{ifndef(box)}
{caption(Таблица {counter(table)[id=numb_tab_sa_rights]} — Права пользователей Портала администратора)[align=right;position=above;id=tab_sa_rights;number={const(numb_tab_sa_rights)}]}
![Права пользователей Портала администратора](./assets/sa_roles-go.png){params[printWidth=140mm;printHeight=210mm;noBorder=true]}
{/caption}
...
Кроме пользовательских учетных записей в {var(sys3)} есть встроенные учетные записи для внутренних запросов между компонентами.
...
Каждый сервис OpenStack использует свою сервисную учетную запись для авторизации запросов к {var(sys3)}. Исключение — Суперадминистратор с учетной записью `admin`.