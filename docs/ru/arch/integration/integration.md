...
Интеграция — синхронизация информации о пользовательских объектах {var(sys2)} с системой, развёрнутой Заказчиком самостоятельно (например, AD, DNS и прочими).
...
В методе используются параметры, приведённые в таблице {counter:table-number}.
...
Создать группу AD для ВМ. Для каждой ВМ создаётся своя группа.
...
В методе используются параметры, приведённые в {linkto(#tab_delete_group_params)[text=таблице %number]}.
...
В методе используются параметры, приведённые в {linkto(#tab_computer_var)[text=таблице %number]}.
...
Структурно информация, передаваемая в целевые DNS, состоит из словаря (таблица 8). При создании объекта создаётся DNS-информация, при удалении — удаляется, при изменении — удаляется и создаётся новая.
...
В методе используются параметры, приведённые в {linkto(#tab_del_rra_params)[text=таблице %number]}.
