...
Управлять {var(sys5)} можно при помощи OpenStack CLI. Для этого выполните подготовительные операции (подробнее — в разделе {linkto(../usage/interfaces_access#use_openstackcli)[text=%text]}), затем — команды, приведённые ниже.
...
где `<ИМЯ_ИЛИ_ID_ШАБЛОНА>` — название или ID шаблона ВМ.
...
где `<ИМЯ_ИЛИ_ID_ШАБЛОНА>` — название или ID шаблона ВМ. Несколько шаблонов перечисляются через пробел.
...
где `<ИМЯ_ИЛИ_ID_ВМ>` — название или ID инстанса (ВМ).
...
* `--flavor <FLAVOR>` — тип ВМ, до которого будет изменён тип ВМ (название или ID типа).
...
где `<ИМЯ_ИЛИ_ID_ВМ> [ <ИМЯ_ИЛИ_ID_ВМ_2> …]` — название или ID инстанса (ВМ); несколько ВМ указываются через пробел.
...
где `<ИМЯ_ИЛИ_ID_ОБРАЗА>` — название или ID образа диска ВМ.
...
где `<ИМЯ_ИЛИ_ID_СНИМКА_ДИСКА>` — название или ID снимка диска.
...
* `--volume <VOLUME>` — диск (название или ID), для которого создаётся снимок. Значение по умолчанию совпадает с `<NAME>`.
...
где `<ИМЯ_ИЛИ_ID_СНИМКА_ДИСКА> [<ИМЯ_ИЛИ_ID_СНИМКА_ДИСКА_2> …]` — название или ID снимка диска, который необходимо удалить; несколько дисков перечисляются через пробел.
...
где `<ИМЯ_ИЛИ_ID_ФАЙЛОВОГО_ХРАНИЛИЩА>` — название или ID файлового хранилища.
...
где `<ИМЯ_ИЛИ_ID_ФАЙЛОВОГО_ХРАНИЛИЩА> [<ИМЯ_ИЛИ_ID_ФАЙЛОВОГО_ХРАНИЛИЩА_2> …]` — название или ID файлового хранилища; несколько значений отделяются пробелом.
...
где `<ИМЯ_ИЛИ_ID_СНИМКА> [<ИМЯ_ИЛИ_ID_СНИМКА_2> …]` — название или идентификатор снимка файлового хранилища; несколько значений разделяются друг от друга пробелом.
...
где `<ИМЯ_ИЛИ_ID_ПОДСЕТИ>` — название или ID подсети.
...
* `--name <NAME>` — новое название подсети.
...
где `<ИМЯ_ИЛИ_ID_ПОДСЕТИ> [<ИМЯ_ИЛИ_ID_ПОДСЕТИ_2> …]` — название или ID подсети; несколько значений отделяются друг от друга пробелом.
...
где `<ИМЯ_ИЛИ_ID_ПОРТА>` — название или ID порта.
...
* `--network <NETWORK>` — сеть (название или ID), для которой создаётся порт.
...
где `<ИМЯ_ИЛИ_ID_ПОРТА> [<ИМЯ_ИЛИ_ID_ПОРТА_2> …]` — название или ID порта; несколько значений отделяются друг от друга пробелом.
...
где `--project <PROJECT>` — проект (название или ID), к которому привязана группа безопасности.
...
где `<ИМЯ_ИЛИ_ID_ГРУППЫ_БЕЗОПАСНОСТИ>` — название или ID группы безопасности.
...
* `--project <PROJECT>` — проект, в котором создаётся группа безопасности (название или ID).
...
* `--remote-group <GROUP>` — удалённая группа безопасности (название или ID).
...
* `--project <PROJECT>` — проект, в котором создаётся правило (название или ID).
...
где `<ИМЯ_ИЛИ_ID_БАЛАНСИРОВЩИКА>` — название или ID балансировщика нагрузки.
...
где `<ИМЯ_ИЛИ_ID_ОБРАБОТЧИКА>` — название или ID обработчика балансировщика нагрузки.
...
где `--loadbalancer <LOAD_BALANCER>` — фильтрация пулов серверов по балансировщику нагрузки (название или ID).
...
где `<ИМЯ_ИЛИ_ID_ПУЛА_СЕРВЕРОВ>` — название или ID пула серверов балансировщика нагрузки.
...
где `<ИМЯ_ИЛИ_ID_МАРШРУТИЗАТОРА>` — название или ID маршрутизатора.
...
где `<ИМЯ_ИЛИ_ID_МАРШРУТИЗАТОРА>` — название или ID маршрутизатора; несколько значений отделяются друг от друга пробелом.

<!--- //[[cli_openstack_floating_ip_manage]] -->
...
где `<IP_АДРЕС_ИЛИ_ID> [<IP_АДРЕС_ИЛИ_ID_2> …]` — название или ID IP-адреса; несколько значений отделяются друг от друга пробелом.
...
где `<НАЗВАНИЕ_ИЛИ_ID_СУБД>` — название или ID СУБД в системе.
...
где `--datastore <DATASTORE>` — СУБД (название или ID), по которой выполнить фильтр.
...
где `<ИМЯ_ИЛИ_ID_ИНСТАНСА_БД>` — название или ID инстанса БД.
...
где `<ИМЯ_ИЛИ_ID_РЕЗЕРВНОЙ_КОПИИ>` — название или ID резервной копии БД.
