...
Перед использованием OpenStack CLI выполните подготовительные операции (подробнее — в разделе {linkto(../interfaces_access#use_openstackcli)[text=%text]}).
...
* `--changes-since <CHANGES_SINCE>` — выводит список ВМ, измененных после указанного момента времени. Указываемое время должно быть в формате `ISO 8061`. Пример: `2016-03-05T06: 27: 59Z`.
...
* `--file <DEST_FILENAME=SOURCE_FILENAME>` — файл, который добавляется в образ ОС. Пример: SSH-ключи.
...
### {appendix-heading(Остановка ВМ)[id=cli_openstack_server_stop; position=prefix]}
...
* `--all-projects` — отобразить снимки всех проектов (доступно только для Администратора {ifdef(box, pg)}{var(sys2)}{/ifdef}{ifndef(box, pg)}{var(sys2_go)}{/ifndef}.
...
* `--state <STATE>` — установить статус для снимка (только для Администратора {ifdef(box, pg)}{var(sys2)}{/ifdef}{ifndef(box, pg)}{var(sys2_go)}{/ifndef}. Возможные значения: `available`, `error`, `creating`, `deleting`, `error-deleting`.
...
## {appendix-heading(Управление VNC-консолью)[id=cli_vnc_console_management; position=prefix]}

{ifndef(cer)}

### {appendix-heading(Получение доступа к VNC-консоли ВМ)[id=cli_vnc_console_getting_access; position=prefix]}
...
### {appendix-heading(Генерация ссылки на VNC-консоль)[id=cli_vnc_console_getting_access; position=prefix]}
...
### {appendix-heading(Просмотр журнала VNC-консоли)[id=cli_openstack_console_log_show; position=prefix]}
...
* `--service-type <SERVICE_TYPE>` — тип сервиса для подсети. Пример: `network:floatingip_agent_gateway`.
...
* `--allowed-cidr <ALLOWED_CIDR>` — разрешенный IP-адрес или маска сети в нотации CIDR; несколько значений указываются с отдельным ключом `--allowed-cidr`. Пример: `--allowed-cidr <ALLOWED_CIDR_1> --allowed-cidr <ALLOWED_CIDR_2>`.
...
* `--external-gateway <NETWORK>` — внешняя сеть (с доступом к сетям вне {ifdef(box, pg)}{var(sys2)}{/ifdef}{ifndef(box, pg)}{var(sys2_go)}{/ifndef}). Обычно используется сеть `ext-net`.
