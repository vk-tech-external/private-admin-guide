# {heading(Работа с узлами для кластеров Ceph)[id=ceph]}

## {heading(Общие сведения)[id=ceph_general_details]}

Управление кластером Ceph на {ifdef(box)}{var(sys3)}{/ifdef}{ifndef(box)}{var(sys3_go)}{/ifndef} осуществляется с помощью плейбука `ansible-openstack/playbooks/ceph-osd-deploy.yml` c использованием основного inventory `vkcloud/vkcloud.yml`.

Основные группы для Ceph кластера, которые требуются для развертывания {ifdef(box)}{var(sys2)}{/ifdef}{ifndef(box)}{var(sys2_go)}{/ifndef}:
...
В состав дистрибутива {ifdef(box)}{var(sys2)}{/ifdef}{ifndef(box)}{var(sys2_go)}{/ifndef} входит версия Ceph octopus. Чтобы проверить версию Ceph, выполните команду `ceph --version`.

=== Первоначальная подготовка

TIP: В примерах ниже используется Ceph кластер из трех нод. Для большей отказоустойчивости рекомендуется создать кластер из 9 нод.
...
+
TIP: Получите значения переменных, выполнив команду `ceph-authtool --gen-print-key` на узле Ceph, и укажите вместо пустых значений, приведенных в примере далее.
+
.Переменные для Ceph
[source, yaml]
----
...
----
+
WARNING: Переменные для ключей доступов к Ceph кластеру, ID кластера и подсети должны совпадать с переменными, указанными в ansible-openstack, в качестве подключения к Ceph, перед развертыванием {ifdef(box)}{var(sys2)}{/ifdef}{ifndef(box)}{var(sys2_go)}{/ifndef}.
...
----
+
. Запустите развертывание Ceph кластера, выполнив команду:
+
[source, console]
----
...
Узел — физический сервер, на котором может быть развернуто несколько сервисов Ceph.
...
1. Отключите службу, отвечающую за работу MGR-сервиса:
   ...
   Если заранее известно, для какого сервера выводится узел, укажите его в команде. Пример для сервера `csn002`:
...
На {ifdef(box)}{var(sys3)}{/ifdef}{ifndef(box)}{var(sys3_go)}{/ifndef} поддержка тонких томов выполняется по умолчанию (параметр `thin_provisioning_support` указан в `rbd` драйвере: `./cinder/volume/drivers/rbd.py`).
