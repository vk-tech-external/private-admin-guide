...
High-IOPS диски на {ifdef(box, pg)}{var(sys3)}{/ifdef}{ifndef(box, pg)}{var(sys3_go)}{/ifndef} — вид хранилища, реализованного поверх нескольких дисков SSD или NVME, подключенных локально к одному или нескольким вычислительным узлам. За счет локального подключения и более простой организации, в отличие от, например, дисков Ceph, достигается большая производительность операций ввода-вывода. Подключение таких дисков к ВМ происходит с помощью протокола iSCSI.

На базовом уровне отказоустойчивость таких дисков может обеспечиваться использованием механизма LVM Mirror.

Ниже приведена настройка дисков для LVM Mirror.

## {heading(Подготовка окружения)[id=highiops_environment_preparation]}

1. Добавьте в `vkcloud.yml` группу `vkcloud_scst_iscsi`, если ее нет.
...
Количество доступных дисков SSD или NVME на узлах может отличаться в зависимости от назначения системы. Для продуктивной системы будет достаточно 6 или 8 дисков, для тестовых случаев подойдет 2 или 4 свободных диска.
...
1. Создайте файловую систему и примонтируйте ее:
...
Здесь `cinder_hiops_use_lvm_mirrors` — параметр, который отвечает за настройку режима `LVM Mirror`.

<err>

Не копируйте пример, приведенный выше, без изменений. Укажите точно, какие диски надо настроить. Во время автоматической настройки не требуется дополнительных подтверждений. Если будут указаны неверные диски, данные на них уничтожатся.
...
Перед запуском указанного выше кода убедитесь в корректности настроек сервера для {ifdef(box, pg)}{var(sys2)}{/ifdef}{ifndef(box, pg)}{var(sys2_go)}{/ifndef}. Должна быть выполнена конфигурация `common.yml`. Диски, соответствующие указанным параметрам, будут автоматически размечены и объединены в массивы. Будут установлены и запущены необходимые компоненты и сервисы.
