# {heading(Миграция ВМ VMware)[id=migration_vm_vmware]}
...
{ifndef(box, pg)}
## {heading(Миграция ВМ из VMware в {var(system_go)})[id=iaas_vm_migration_from_vmware]}
{/ifndef}
...
Подробные инструкции приведены в документе **Руководство пользователя {ifdef(box, pg)}{var(sys)}{/ifdef}{ifndef(box, pg)}{var(sys_go)}{/ifndef}** в разделе **Облачные вычисления** → **Миграция**.
...
{ifndef(box, pg)}
## {heading(Миграция ВМ из {var(system_go)} в VMware)[id=iaas_vm_migration_in_vmware]}
{/ifndef}

Для миграции ВМ из {ifdef(box, pg)}{var(sys2)}{/ifdef}{ifndef(box, pg)}{var(sys2_go)}{/ifndef} в VMware выполните шаги:
...
* Портал самообслуживания (подробнее — в документе **Руководство пользователя {ifdef(box, pg)}{var(sys)}{/ifdef}{ifndef(box, pg)}{var(sys_go)}{/ifndef}** в разделе **Облачные вычисления** → **Виртуальные машины** →**Управление ВМ** → **Запуск, перезапуск и остановка ВМ**).

{ifndef(cer)}
* OpenStack CLI (подробнее — в разделе {linkto(../../../../iaas_main/iaas_vm/iaas_vm_managing#vm_start_restart_stop_cli)[text=Запуск, перезапуск и остановка ВМ]}).
{/ifndef}
...
Чтобы клонировать диск, используйте Портал самообслуживания (подробнее — в документе **Руководство пользователя {var(system)}** в разделе **Облачные вычисления** → **Диски** → **Другие операции с дисками инстансов** → **Портал самообслуживания** → **Клонирование диска**).
...
Создайте образ диска с помощью Портала самообслуживания (подробнее — в документе **Руководство пользователя {var(system)}** в разделе **Облачные вычисления** → **Образы** → **Портал самообслуживания** → **Создание образа из диска**).
