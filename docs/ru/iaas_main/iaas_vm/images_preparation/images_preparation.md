# {heading(Подготовка образа диска ВМ для загрузки на {var(sys4)})[id=images_preparation]}

<err>

Виртуальные машины, созданные {ifdef(box, pg)}на {var(sys3)}{/ifdef}{ifndef(box, pg)}в {var(sys3_go)}{/ifndef}, используют эмуляцию BIOS. Эмуляция UEFI не поддерживается.

</err>

## {heading(Создание образа ВМ с ОС Windows)[id=images_preparation_windows]}

На {var(sys3)} поддерживается миграция ОС Windows серверных версий:
...
1. Скачайте ISO-образ операционной системы, для которого планируется миграция на {var(sys4)}. Рекомендуется использовать en-US версию образа.
1. Выполните подготовительные операции (подробнее — в разделе {linkto(../../../interfaces_access#prerequisites_openstack_cli_config)[text=%text]}).
...
1. Импортируйте образ на {var(sys4)}:
...
1. Импортируйте образ диска локальной ВМ на {var(sys4)}:
   ...
   1. Установите на локальную ВМ клиент OpenStack (подробнее — в разделе {linkto(../../../interfaces_access#prerequisites_openstack_cli_config)[text=%text]}).
   1. Загрузите образ `Arch.raw` на {var(sys4)} под именем `Arch`:
...
Packer позволяет сконструировать новый образ ВМ на основе базового образа с желаемой гостевой ОС и любого из предустановленных на {var(sys3)} шаблонов конфигурации. Параметры нового образа задаются с помощью конфигурационного файла Packer. В качестве примера будут использованы:
...
1. Загрузите базовый образ на {var(sys4)}.
1. Создайте конфигурационный Packer-файл:
   ...
   1. Создайте файл `altlinux.pkr.hcl`. Укажите имя выбранного шаблона конфигурации в параметре `flavor` и зону доступности в параметре `volume_availability_zone`.
      ...
      Подробная информация о синтаксисе конфигурационного файла в [официальной документации Packer](https://developer.hashicorp.com/packer/docs/templates/hcl_templates).
...
1. Загрузите подготовленный образ на {var(sys4)}:
...
1. Загрузите образ на {var(sys4)}.

   Загрузите образ `opensuse-15.3.raw` на {var(sys4)} под именем `Opensuse`:
