# {heading(Подготовка образа диска ВМ для загрузки в {var(sys4_go)})[id=images_preparation]}

Подготовка образа диска ВМ для его загрузки в {var(sys4_go)} может выполняться разными способами. Далее описаны следующие варианты подготовки образа:

* Утилита [OpenStack Builder](https://www.packer.io/docs/builders/openstack) в составе Packer (подробная информация приведена в разделе {linkto(#packer_image_prepare)[text=%text]}).
* Ручная локальная подготовка образа через существующую ВМ в {var(sys3_go)} (подробная информация приведена в разделе {linkto(#manual_image_prepare)[text=%text]}).
* Утилита [diskimage-builder](https://opendev.org/openstack/diskimage-builder) (подробная информация приведена в разделе {linkto(#diskimage_image_prepare)[text=%text]}).

<err>

В приведенных инструкциях дополнительно описаны шаги загрузки подготовленного образа в {var(sys4_go)}.
...
Образ создается на основе **существующего** образа в {var(sys3_go)}.

</err>

### {heading(Предусловия)[id=packer_image_prepare_prerequisites]}
...
### {heading(Основные шаги)[id=packer_image_prepare_step]}

Чтобы подготовить образ с помощью `OpenStack Builder`:

1. Скачайте и установите [Packer](https://www.packer.io/).
1. Создайте или скачайте OpenRC-файл с доступами к облаку (подробнее — в примере файла в разделе {linkto(#prerequisites_openstack_cli_config)[text=%text]}).
1. Создайте директорию со скриптами из раздела {linkto(#packer_image_prepare_prerequisites)[text=%text]}.
...
После успешного выполнения всех шагов в {var(sys3_go)} появится загруженный образ. Посмотреть образ можно в Портале самообслуживания или Портале администратора на странице **Образы**.

### {heading(Перенос образа)[id=packer_image_prepare_transfer]}
...
### {heading(Предусловия)[id=manual_image_prepare_prerequisites]}
...
### {heading(Основные шаги)[id=manual_image_prepare_step]}
...
1. Выполните экспорт переменных для доступа к OpenStack API любым удобным способом (подробнее — в разделе {linkto(#use_openstackcli)[text=%text]}).
1. Загрузите образ:
...
После успешного выполнения шагов будет создан файл (в примере выше — `Arch.img`), загруженный в {var(sys4_go)}.

### {heading(Дополнительные шаги)[id=manual_image_prepare_additional_step]}
...
### {heading(Предусловия)[id=diskimage_image_prepare_prerequisites]}
...
### {heading(Основные шаги)[id=diskimage_image_prepare_step]}
...
После успешного выполнения шагов будет создан файл (в примере выше — `opensuse-15.3.img`), загруженный в {var(sys4_go)}.

### {heading(Кастомизация)[id=diskimage_image_prepare_custom]}
