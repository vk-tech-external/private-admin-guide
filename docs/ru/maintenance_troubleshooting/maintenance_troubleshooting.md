...
1. Предоставьте права группе пользователей (подробнее — в разделе {linkto(../usage/users_management#superadmin_project_access_settings)[text=%text]}).
...
1. Выполните подготовительные операции для подключения к OpenStack CLI (подробнее — в разделе {linkto(../usage/interfaces_access#use_openstackcli_admin)[text=%text]}).
...
1. Получите список состояний сервисов:
   ...
   1. Проверьте логи и перезагрузите отключённые сервисы:
