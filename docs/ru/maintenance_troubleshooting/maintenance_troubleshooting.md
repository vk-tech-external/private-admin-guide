...
<!--- //В данном разделе представлены описания действий, предназначенных для базовой диагностики неисправностей {var(sys2)} и входящих в неё сервисов и компонентов. Если вы не нашли здесь интересующую вас проблему, обращайтесь к статьям базы знаний (ссылка на раздел БЗ по траблшутингу). -->

## {heading(Диагностика {var(sys2)})[id=diagnostics]}

<!--- //#todo Уточнить какие ещё бывают виды диагностики. Например, расширенная и т.п. -->

Порядок выполнения быстрой диагностики неисправностей {var(sys2)}:

1. Проверьте работоспособность сервисов {var(sys2)} (подробнее — в разделе {linkto(#common_sys_checking)[text=%text]}).
1. Убедитесь, что доступны основные веб-интерфейсы {var(sys2)}:
   ...
   {ifdef(cer)}
   1. Портал управления доступом.
   1. Портал администратора {var(sys2)}.
...
Вместо местозаполнителей укажите значения соответствующих переменных из файлов `~/inventory/<ENV>/group_vars/<ENV>/vars.yml` и `~/inventory/<ENV>/group_vars/<ENV>/vault.yml`, где `ENV` — название окружения.
...
1. Предоставьте права группе пользователей (подробнее — в разделе {linkto(../usage#superadmin_project_access_settings)[text=%text]}).
...
* используйте данные учётных записей, реквизиты которых есть в Портале управления доступом (подробнее — в разделе {linkto(#keycloak_checking)[text=%text]}).
...
1. Перейдите в раздел **Облачные вычисления**, активируйте сервисы согласно предложенной инструкции и создайте ВМ. Шаги по созданию ВМ приведены в **Руководстве пользователя {var(system)}**.
...
Для обслуживания узлов кластера предусмотрен механизм их временного отключения. Инструкции по выводу узлов из кластера приведены в соответствующих разделах:
...
### {heading(Проверка работы сервисов {var(sys2)})[id=checking_services]}
...
1. Просмотр состояния отдельного сервиса RabbitMQ:
   ...
   где `<NAME>` — имя целевого сервиса RabbitMQ.
...
1. На деплой-ноде найдите пароль от целевой БД (в данном случае — MySQL; в команде ниже `<ENV>` — название окружения):
...
1. Выполните подготовительные операции для подключения к OpenStack CLI по инструкции {linkto(../usage/interfaces_access#use_openstackcli_admin)[text=%text]}.
...
1. Получите список состояний сервиса, выполнив команду:
   ...
   Поле **Alive** должно иметь значение `True`, поле **State** — `UP` (для всех сетевых агентов). Если есть сервисы с отключенным агентом, выполните шаги:
