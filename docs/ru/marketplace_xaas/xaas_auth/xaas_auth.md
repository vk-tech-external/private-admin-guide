# {heading(Авторизация в Marketplace под учетной записью администратора {var(sys2)})[id=xaas_auth]}

Чтобы авторизоваться в Marketplace под учетной записью администратора {var(sys2)}, выполните запрос с параметрами из {linkto(#tab_authorization_request_parameters)[text=таблицы %number]}.
...
* `email` — электронная почта учетной записи администратора {var(sys2)}.
* `password` — пароль учетной записи администратора {var(sys2)}
|===
{/caption}
