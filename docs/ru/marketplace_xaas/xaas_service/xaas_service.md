...
* Параметр `slug`. Используется в запросе, чтобы посмотреть отчет о статистике потребления сервиса (подробнее — в разделе {linkto(../../marketplace_xaas/xaas_report#xaas_report)[text=%text]})
...
ID брокеров Marketplace отображаются в ответе на запрос списка зарегистрированных брокеров (подробнее — в разделе {linkto(../../marketplace_xaas/xaas_broker#xaas_broker_list)[text=%text]})
