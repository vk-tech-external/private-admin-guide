...
Сервисный ключ позволяет загружать image-based приложения в связанный тенант image-based брокера.

Чтобы создать сервисный ключ, выполните запрос с параметрами из {linkto(#tab_service_key_creation)[text=таблицы %number]}. Передайте сервисный ключ поставщику.
...
* `test_ns` — список тестовых пространств имен Marketplace, в которых будет доступен сервис. Имена задаются администратором, должны совпадать со значениями, задаваемыми при регистрации тенанта image-based брокера (подробнее — {linkto(../../marketplace_xaas/xaas_broker#xaas_broker_register)[text=%text]}).
* `prod_ns` — список открытых пространств имен Marketplace, в которых будет доступен сервис. Имена задаются администратором, должны совпадать со значениями, задаваемыми при регистрации тенанта image-based брокера (подробнее — {linkto(../../marketplace_xaas/xaas_broker#xaas_broker_register)[text=%text]}).
...
* `email` — электронная почта пользователя, которая назначается владельцем сервисного ключа. Укажите значение, полученное от поставщика
...
* Список тестовых пространств имен Marketplace.
* Список открытых пространств имен Marketplace.
...
|1. Укажите обязательные параметры:
   ...
   * `email` — электронная почта пользователя-владельца сервисного ключа.
...
1. Укажите один или несколько параметров, которые нужно изменить:
   ...
   * `test_ns` — список тестовых пространств имен Marketplace, в которых будет доступен сервис.
   * `prod_ns` — список открытых пространств имен Marketplace, в которых будет доступен сервис.
