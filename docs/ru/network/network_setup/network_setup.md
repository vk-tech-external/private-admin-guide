...
<!--- #todo на раздел **Создание внешней сети с доступом в корпоративную сеть** есть ссылка в Руководстве по установке Private Cloud -->

<!--- // исходные данные https://confluence.vk.team/pages/viewpage.action?pageId=839321127 -->

{ifdef(box, pg)}{var(sys1)}{/ifdef}{ifndef(box, pg)}{var(sys1_go)}{/ifndef} поддерживает варианты сетей:
...
Настройки доступа по умолчанию запрещают создание внешней сети под любым пользователем, кроме Администратора {ifdef(box, pg)}{var(sys2)}{/ifdef}{ifndef(box, pg)}{var(sys2_go)}{/ifndef}.
...
* `registry-<ПОРТАЛ_САМООБСЛУЖИВАНИЯ>:5010` — docker-registry на управляющих узлах. В нем хранятся артефакты для развертывания кластера.
...
При задании VLAN ID, например `123`, для пользовательской сети убедитесь, что у ОС нет прямых интерфейсов, которые работают с тем же VLAN ID, таких как `bond0.123`. Иначе работоспособность сети будет нарушена из-за конфликтов на уровне L2.
...
1. Измените настройки в конфигурационных файлах, расположенных в директории `<vkcloud>/group_vars/` на деплой-ноде:
   ...
   1. В файле `<vkcloud>/networks.yml` задайте параметры конфигурации для внешней Neutron-сети:
   ...
   1. В следующих файлах задайте настройки мостов для выхода ВМ во внешнюю сеть: br-vlan интерфейс для выхода с VLAN, br-ex интерфейс для выхода без VLAN (создается как дополнение, без какого-либо присвоенного IP):
      ...
      * `<vkcloud>_compute/networks.yml` — файл с настройками вычислительных узлов, на которых будут запускаться ВМ.
      ...
      * `<vkcloud>_dhcp/networks.yml` — файл с настройками DHCP:
      ...
      {ifndef(cer)}
      * `<vkcloud>_octavia/networks.yml` — файл с настройками балансировщика Octavia:
      ...
      * `<vkcloud>_network/networks.yml` — файл с настройками Neutron-сети:
...
1. Создайте подсеть в только что созданной сети:
   ...
   * `<ПОДСЕТЬ>` — адрес подсети, которую вам выдали в этом VLAN. Пример: `172.25.120.0/24`.
   * `<ПУЛ_АДРЕСОВ>` — пул IP-адресов для ВМ. Не забудьте оставить место для шлюза и двух DHCP-серверов. Пример: `start=172.25.120.4,end=172.25.120.245`.

### {heading(Подключение пользовательских сетей к внешней сети)[id=connecting_user_networks_to_external_networks]}

<warn>

Настройки доступа по умолчанию запрещают подключение к внешней сети внутренних сетей под любым пользователем, кроме Администратора {ifdef(box, pg)}{var(sys2)}{/ifdef}{ifndef(box, pg)}{var(sys2_go)}{/ifndef}.
...
1. Войдите в Портал самообслуживания с использованием учетной записи Администратора {ifdef(box, pg)}{var(sys2)}{/ifdef}{ifndef(box, pg)}{var(sys2_go)}{/ifndef}.
...
Подключить внешнюю сеть к пользовательской можно при создании сети в Портале самообслуживания (подробнее — в разделе {linkto(../../network/network_managed#network_creating)[text=%text]}). На этапе создания сети активируйте опцию **Доступ в интернет**.

</info>

Чтобы пройти аналогичные шаги в OpenStack CLI выполните подготовительные операции для использования OpenStack CLI (подробнее — в разделе {linkto(../../interfaces_access#use_openstackcli_admin)[text=%text]}), затем — команды:
...
Присоединение внешней сети в качестве приватной к роутерам (пример команды: `openstack router add subnet`) приводит к нарушению связности до внешнего шлюза в этой сети.
...
1. Отключите использование внешней глобальной сети. В файле `<vkcloud>/group_vars/vkcloud/networks.yml` задайте параметру `create_external_network` значение `false`.
1. В файле `<vkcloud>/group_vars/vkcloud/networks.yml` в переменной `postdeploy_provider_network` задайте параметры новых сетей.

   {caption(Пример фрагмента файла `networks.yml`)[align=left;position=above]}
   ```yaml
...
1. Запустите плейбук:
   ...
   После создания новой внешней сети проекта, доступной только в нем, создайте шаблон кластера Kubernetes (подробнее — в разделе {linkto(../../paas/cluster_template#cluster_template_add)[text=%text]}).

   </warn>

Чтобы создать внешнюю сеть для проекта с помощью OpenStack CLI:
...
{ifdef(box, pg)}
Удалять внешние сети можно с помощью OpenStack CLI, выполнив команду из раздела {linkto(../../cli_commands#cli_openstack_network_delete)[text=%text]}.
{/ifdef}

{ifndef(box, pg)}
Удалять сети и сетевые подключения можно через Портал самообслуживания. Шаги по выполнению операций приведены в документе **Руководство пользователя {var(system_go)}** в разделе **Виртуальные сети** → **Сети** → **Управление сетями, подсетями и портами**.
{/ifndef}
...
Чтобы добавить подсеть адресов во внешнюю сеть, создайте новую подсеть в этой внешней сети (подробнее — в разделе {linkto(../../network/network_managed#subnet_creating)[text=%text]}).
