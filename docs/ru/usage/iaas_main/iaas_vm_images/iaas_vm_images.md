...
Чтобы просмотреть список образов при помощи OpenStack CLI:
...
Образ можно создать путём загрузки файла в формате `RAW` или создания из существующего диска.

<err>

Публичные образы ВМ, взятые вне {var(sys2)}, необходимо предварительно подготовить для загрузки в {var(sys4)} (подробнее — в разделе {linkto(../../../administration/images_preparation#images_preparation)[text=%text]}). Иначе может не работать часть функций, например, резервное копирование.
...
1. Нажмите кнопку **Добавить**. Задайте параметры образа:
   ...
   * Общий доступ к образу:
      ...
      * **Защищенный** — признак защищённости образа. В случае опции **Да** его смогут удалить только пользователи с соответствующими правами.
