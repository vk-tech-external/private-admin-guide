...
### {heading(Создание образа)[id=iaas_vm_images_creating]}

#### {heading(Портал администратора)[id=iaas_vm_images_creating_sa]}
...
1. Нажмите кнопку **Создать образ** и дождитесь нотификации об успешном выполнении операции.
...
1. Добавьте пользовательские метаданные (пары **ключ** — **значения**), нажимая кнопку **Добавить** для каждой пары значений:
