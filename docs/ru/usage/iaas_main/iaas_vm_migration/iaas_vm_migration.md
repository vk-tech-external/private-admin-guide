...
Для миграции ВМ из {var(sys2)} в VMware выполните шаги, описанные ниже:
...
{ifndef(cer)}
* OpenStack CLI (подробнее — в разделе {linkto(#vm_disk_clone_cli)[text=Клонирование диска]}).
{/ifndef}
...
1. Запустите ВМ и проверьте, что ВМ запущена.
