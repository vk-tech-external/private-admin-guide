...
1. Войдите под пользователем `centos`, если этого не было сделано ранее, выполните команду:

   ```console
   $ sudo su - centos
   ```
 
1. Если имя узла неизвестно, получите список узлов, выполнив команду:
...
1. Перейдите на требуемый узел с помощью команды:
...
По умолчанию для переключения между узлами настроен беспарольный вход для пользователя `centos` (присутствующего на всех узлах). Но может требоваться корректная ключевая пара (точнее закрытый SSH ключ).
...
1. Дальше можно выполнять запросы к OpenStack. Для проверки работоспособности подключения к OpenStack CLI, выполните команду:
...
1. Проверьте подключение OpenStack CLI, выполнив простейшую команду, например:
...
Перед выполнением запроса к REST API {var(sys2)} требуется получение токена авторизации.

<warn>

Токен авторизации действителен в течение одного часа. Все выпущенные токены остаются действительными до завершения времени действия.

</warn>

{ifndef(box)}
Получить токен авторизации можно в Портале администратора, Портале самообслуживания:
{/ifndef}
{ifdef(box)}
Получить токен авторизации можно в Портале администратора, Портале самообслуживания или с помощью запроса к OpenStack CLI:
{/ifdef}
...
1. В нижней части экрана скопируйте значение параметра **Токен для доступа к API**.
...
{/ifdef}
{caption(Пример использования токена авторизации в запросах)[align=left;position=above]}
```console
# curl -X POST -H "X-Auth-Token: <ТОКЕН>" <ТЕКСТ_ЗАПРОСА>
```
{/caption}

### {heading(Перевыпуск токена авторизации)[id=http_api_reissue_auth_token]}

После завершения времени действия перевыпустить токен авторизации можно в Портале администратора и Портале самообслуживания:
...
### {heading(Точки подключения (Endpoints))[id=http_api_endpoints]}

Точки подключения к API можно получить в Портале самообслуживания или с помощью команды OpenStack CLI.
...
{caption(Команда OpenStack CLI)[align=left;position=above]}
```console
...
{ifdef(box)}
1. Создайте служебную учётную запись. Подровнее в инструкции —  {linkto(../../usage/users_management#service_account)[text=%text]}.
{/ifdef}
...
1. На рабочей станции создайте файл `~/.terraformrc` со следующим содержимым:
   ...
   где `path` — путь, по которому будет распакован архив с провайдерами для работы с OpenStack, например, `/home/redos/terraform_providers/`.
...
1. Скачайте архив, расположенный по адресу `<DEPLOY_NEXUS_IP>:<PORT>/repository/share/registry.terraform.io.tar.gz`. Распакуйте архив по пути, заданному в `path` в файле `~/.terraformrc`:
   ...
   где `<DEPLOY_NEXUS_IP>` — адрес Deploy-сервера с запущенным Nexus.
...
1. При необходимости введите токен API.
   ...
   Токен API действует 1 час.

   </warn>
   
1. Перейдите на вкладку **Terraform**. Скачайте основной файл конфигурации Terraform и файл конфигурации зеркала Terraform, нажав одноименные кнопки.
...
1. Cкопируйте файл `vkcs_provider.tf` в рабочую директорию, из которой планируете работать с проектом.
...
В директории, из которой планируете работать с проектом, выполните команду `terraform init`.
...
1. Создайте манифест ресурсов в рабочей директории — например, конфигурацию для создания виртуальной машины.
