...
1. Выполните вход в Портал администратора с учетными данными Администратора {var(sys2)}.
...
Значение переменной `session_expire_period` задает следующие ограничения:
...
После получения токена доступа (подробнее — в разделе {linkto(#http_api_get_auth_token)[text=%text]}) возможна работа с {var(sys5)} через REST API. Список методов для сервисов OpenStack приведен в [официальной документации](https://docs.openstack.org/api-quick-start/#current-api-versions).
...
{ifdef(box)}
1. Создайте служебную учетную запись. Подробнее — в разделе {linkto(../../usage/users_management#service_account)[text=%text]}.
{/ifdef}
