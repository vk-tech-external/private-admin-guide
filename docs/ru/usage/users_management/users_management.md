...
1. Задайте параметры:
   ...
   * **Single Sign-On Service URL** — укажите адрес сервиса единой точки входа, например, `https://10.28.227.59/adfs/ls/`.
   * **Single Logout Service URL** — укажите адрес сервиса единой точки выхода, например, `https://10.28.227.59/adfs/ls/`.
...
## {heading(Создание служебной (сервисной) учетной записи)[id=service_account]}
...
1. Создайте конфигурационный файл:
   ...
   В качестве значения `ДОМЕННОЕ_ИМЯ_ПРОЕКТА` используйте значение параметра `name` в выводе команды:
...
Описание ролей пользователей {var(sys2)} приведено в разделе {linkto(../../arch/arch-roles#arch-roles)[text=%text]}.
...
## {heading(Двухфакторная аутентификация)[id=superadmin_2fa]}
...
////
$ sudo tt connect <BREEZE_NAME>
unix/:/var/run/tarantool/<BREEZE_NAME>.control> breeze.api.services.superadmin_users.users.search({search_email = '<USER_EMAIL>'})
////
-->
...
{caption(Пример ожидаемого результата)[align=left;position=above]}
```console
- [200, {'list': [{'domain': 'sa_admins', 'enabled': true, 'uid': <USER_ID>, 'id': '<OpenStack_ID>',
  'ctime': <TIMESTAMP>, 'email': '<USER_EMAIL>', 'roles': [<ROLES_LIST>], 'mfa': {'email': {
...
Если параметр `mfa` пуст, пересоздайте пользователя (подробнее — в разделе {linkto(#recreate_superadmin_2fa)[text=%text]}).
...
1. Удалите пользователя:
   ...
   ```console
   unix/:/var/run/tarantool/<BREEZE_NAME>.control> breeze.api.services.superadmin_users.users.delete({uid = '<USER_ID>'})
   ---
   - [200, {'domain': 'sa_admins', 'enabled': true, 'uid': <USER_ID>, 'id': '<OpenStack_ID>', 
     'ctime': <TIMESTAMP>, 'email': '<USER_EMAIL>', 'roles': [<ROLES_LIST>], 'mfa': {}}]
   ```
...
1. Повторно предоставьте пользователю доступ к проекту (подробнее — в разделе {linkto(#superadmin_access_settings)[text=%text]}).
...
Учетная запись блокируется, если пользователь не входил в {var(sys4)} более 45 дней.
...
* Настроить ограничение времени бездействия пользователя (подробнее — в разделе {linkto(../../usage/interfaces_access#session_expire_period)[text=%text]}).
...
1. Перейдите на деплой-ноду по SSH:
   ...
   Здесь `<USERNAME>` — логин, созданный при инсталляции {var(sys2)} (например, `redos`).
