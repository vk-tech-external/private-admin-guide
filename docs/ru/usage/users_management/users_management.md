...
При входе пользователя в {var(sys4)} средствами Подсистемы управления доступом выполняется поиск соответствующей учётной записи в собственном хранилище. Если учётная запись не найдена, поиск продолжается в преднастроенных внешних базах данных. Вход в {var(sys4)} разрешается, если учётная запись пользователя найдена.
...
1. Задайте параметры подключения к LDAP-провайдеру:
   ...
   * **Bind DN** — учётная запись пользователя, которая будет использоваться для доступа к LDAP-провайдеру, например, `CN=Administrator`, `CN=Users`, `DC=test`, `DC=local`. Учётная запись должна иметь права администратора.
...
## {heading(Создание служебной (сервисной) учётной записи)[id=service_account]}
...
Все учётные записи пользователей расположены в Портале администратора, раздел **Управление доступом**.
...
* `<USER_ID>` — UID учётной записи пользователя.
...
## {heading(Блокировка и разблокировка учётной записи)[id=superadmin_user_role_block_unblock]}
...
1. Перезапустите плейбук для сервиса блокировки:
   ...
   Для перезапуска пода и применения новых настроек сервиса блокировки требуется 1 час. Обычно перезапуск идёт через cron в 00 минут каждого часа.
...
Для каждой учётной записи пользователя единовременно может быть запущен только один сеанс доступа.
