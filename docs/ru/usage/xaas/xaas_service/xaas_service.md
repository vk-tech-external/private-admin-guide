...
* Параметр `slug`. Используется в запросе, чтобы посмотреть отчёт о статистике потребления сервиса (подробнее — в разделе {linkto(../../../usage/xaas/xaas_report#xaas_report)[text=%text]})
