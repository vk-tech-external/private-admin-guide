...
* `test_ns` — список тестовых пространств имен Marketplace, в которых будет доступен сервис. Имена задаются администратором, должны совпадать со значениями, задаваемыми при регистрации тенанта image-based брокера (подробнее — {linkto(../../../usage/xaas/xaas_broker#xaas_broker_register)[text=%text]}).
* `prod_ns` — список открытых пространств имен Marketplace, в которых будет доступен сервис. Имена задаются администратором, должны совпадать со значениями, задаваемыми при регистрации тенанта image-based брокера (подробнее — {linkto(../../../usage/xaas/xaas_broker#xaas_broker_register)[text=%text]}).
...
|1. Укажите обязательные параметры:
   ...
   * `email` — почта пользователя-владельца сервисного ключа.
