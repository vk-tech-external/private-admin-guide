...
* `test_ns` — список тестовых пространств имён Marketplace. Имена задаются администратором.
* `prod_ns` — список открытых пространств имён Marketplace. Имена задаются администратором.
...
* `users_emails` — список email пользователей, добавляемых в тестовые пространства имён `test_ns`
...
* К тенанту image-based брокера будут привязаны тестовые и открытые пространства имён, в которых будут доступны загружаемые сервисы.

   <info>

   Если указанных пространств имён не было на момент выполнения API-запроса, они будут созданы.

   </info>
* В список проектов поставщиков image-based приложений будут добавлены OpenStack PID, указанные в `pids`.
* В тестовые пространства имён будут добавлены пользователи, указанные в `users_emails`.
