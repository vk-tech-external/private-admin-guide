...
Подготовка образа диска ВМ для его загрузки в {var(sys4)} может выполняться разными способами. Далее описаны следующие варианты подготовки образа:


* Утилита [OpenStack Builder](https://developer.hashicorp.com/packer/integrations/hashicorp/openstack/latest/components/builder/openstack) в составе Packer (см. раздел {linkto(#packer_image_prepare)[text=%text]}).
* Ручная локальная подготовка образа через существующую ВМ в {var(sys3)} (см. раздел {linkto(#manual_image_prepare)[text=%text]}).
* Утилита [diskimage-builder](https://opendev.org/openstack/diskimage-builder) (см. раздел {linkto(#diskimage_image_prepare)[text=%text]}).

<err>

В приведенных инструкциях также описаны шаги загрузки подготовленного образа в {var(sys4)}.
...
Также для запуска используется ряд скриптов, пример для выполнения в Ubuntu описан ниже (будет использоваться для выполнения основных шагов).
...
### {heading(Основные шаги)[id=packer_steps]}

Чтобы подготовить образ с помощью `OpenStack Builder`, выполните шаги:

1. Скачайте и установите [Packer](https://www.packer.io).
1. Создайте / скачайте openrc-файл с доступами к облаку (см. пример файла в разделе {linkto(../../../usage_administration/interfaces_access#prerequisites_openstack_cli_config)[text=%text]}).
...
1. Заполните конфигурационный файл `ubuntu-ansible-packer.json`:
   ...
   * `build_flavor` — наименование или идентификатор шаблона ВМ (можно узнать с помощью команды `openstack flavor list`).
   * `int_network` — ID внутренней сети, которая поддерживает плавающие IP-адреса.
   * `ext_network` — ID внешней сети (по умолчанию `external`, можно узнать с помощью команды `openstack network list --long | grep External`).
   * `image_name` — название создаваемого образа.
   * `source_image_name` — наименование образа, на основе которого будет создан новый образ.
...
### {heading(Предусловия)[id=manual_prerequisites]}
...
### {heading(Основные шаги)[id=manual_steps]}

Чтобы подготовить образ вручную, выполните шаги:

1. Установите целевую ОС на ВМ.
1. Перейдите на ВМ по SSH.
1. Установите пакет `cloud-init` (см. [официальную документацию](https://cloud-init.io)).
1. Включите автозапуск `cloud-init` (например, с помощью linux-команды `systemctl enable cloud-init`). При необходимости настройте конфигурационный файл `/etc/cloud/cloud.cfg`.
1. Установите пакет `qemu-guest-agent` (см. [официальную документацию](https://qemu-project.gitlab.io/qemu/interop/qemu-ga.html)).
1. Включите автозапуск `qemu-guest-agent` (например, с помощью linux-команды `systemctl enable qemu-guest-agent`).
1. Выполните дополнительные настройки при необходимости (см. раздел {linkto(#manual_add_steps)[text=%text]}).
1. Выключите локальную ВМ.
1. Выполните экспорт переменных для доступа к OpenStack API любым удобным способом (см. раздел {linkto(../../../usage_administration/interfaces_access#use_openstackcli)[text=%text]}).
1. Загрузите образ:
...
### {heading(Дополнительные шаги)[id=manual_add_steps]}

1. Выключите все ресурсы (`datasources`) в `cloud-init`, кроме OpenStack — создайте файл `/etc/cloud/cloud.cfg.d/15_datasource.cfg`:
...
1. Выполните очистку ВМ — последовательно выполните команды (пример linux-команд):
...
С помощью утилиты `diskimage-builder` можно собрать ограниченный набор образов; список поддерживаемых образов доступен в [официальной документации](https://virtualenv.pypa.io/en/latest/installation.html).
...
### {heading(Предусловия)[id=diskimage_prerequisites]}

* Поддержка Python-библиотек.
* Выход в интернет.
* Установленный пакет `qemu-utils` (можно установить, например, с помощью ubuntu-команды `sudo apt install qemu-utils`).
* Не менее 3 ГБ свободного пространства.

### {heading(Основные шаги)[id=diskimage_steps]}

Чтобы подготовить образ с помощью утилиты `diskimage-builder`, выполните шаги:


1. Установите виртуальное окружение python virtualenv (см. [официальную документацию](https://virtualenv.pypa.io/en/latest/installation.html)).
...
### {heading(Кастомизация)[id=diskimage_custom]}

С помощью `diskimage-builder` возможна донастройка целевой ОС, подробнее см. в [официальной документации](https://docs.openstack.org/diskimage-builder/latest/elements.html).