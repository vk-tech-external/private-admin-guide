...
Чтобы описать {linkto(../../../../xaas_instructions/xaas_vendor_index#xaas_wizard)[text=%text]}, в файле `plans/<PLAN_NAME>/display.yaml` задайте параметры, приведенные в {linkto(#tab_plans)[text=таблице %number]}.
...
|entities
|Описывает элементы инфраструктуры {var(sys2)}, чтобы в мастере конфигурации тарифного плана отображался расчет их стоимости:
...
1. В `parameters` для каждой тарифной опции задайте параметр `name` — имя ее YAML-файла.
