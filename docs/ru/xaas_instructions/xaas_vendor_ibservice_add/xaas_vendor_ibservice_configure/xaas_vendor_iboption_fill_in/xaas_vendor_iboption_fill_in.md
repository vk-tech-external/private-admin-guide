...
Каждый отдельный YAML-файл соответствует одной тарифной опции. В нём описываются настройки тарифной опции, а для платных опций — стоимость.
...
![Тарифная опция-константа типа integer](./assets/xaas_option_int_const_example.png){params[width=50%]}
{/caption}
...
![Тарифная опция типа integer c выбором значения из списка](./assets/xaas_option_int_enum_example.png){params[width=50%]}
{/caption}
...
![Бесплатная тарифная опция типа integer с шагом изменения 1](./assets/xaas_option_int_with_step_standart.png)
{/caption}
...
![Бесплатная тарифная опция типа integer с шагом изменения 1, значение увеличено на 1 шаг](./assets/xaas_option_int_with_step_standart1.png)
{/caption}
...
1. В секции `schema` задайте параметры:
   ...
   * `default` — значение по умолчанию (опционально). Задаётся относительно стандартного значения опции:
      ...
      * Не указывайте параметр `default` или укажите `0`, чтобы значение по умолчанию было равно стандартному значению ({linkto(#pic_xaas_option_int_with_user_step_free)[text=рисунок %number]}).

         {caption(Рисунок {counter(pic)[id=numb_pic_xaas_option_int_with_user_step_free]} — Бесплатная тарифная опция типа integer с пользовательским шагом изменения (billing.base = 25, schema.default = 0, billing.unit.size = 100))[align=center;position=under;id=pic_xaas_option_int_with_user_step_free;number={const(numb_pic_xaas_option_int_with_user_step_free)}]}
         ![Бесплатная тарифная опция типа integer с пользовательским шагом изменения](./assets/xaas_option_int_with_user_step_free.png)
         {/caption}
     
      * Укажите `n`, чтобы значение по умолчанию рассчитывалось по формуле на основе стандартного значения и значения `n` ({linkto(#pic_xaas_option_int_with_user_step_free1)[text=рисунок %number]} и {linkto(#pic_xaas_option_int_with_user_step_free2)[text=рисунок %number]}):

         ```
         billing.base + n * billing.unit.size
         ```

         {caption(Рисунок {counter(pic)[id=numb_pic_xaas_option_int_with_user_step_free1]} — Бесплатная тарифная опция типа integer с пользовательским шагом изменения (billing.base = 25, schema.default = 1, billing.unit.size = 100))[align=center;position=under;id=pic_xaas_option_int_with_user_step_free1;number={const(numb_pic_xaas_option_int_with_user_step_free1)}]}
         ![Бесплатная тарифная опция типа integer с пользовательским шагом изменения](./assets/xaas_option_int_with_user_step_free1.png)
         {/caption}

         {caption(Рисунок {counter(pic)[id=numb_pic_xaas_option_int_with_user_step_free2]} — Бесплатная тарифная опция типа integer с пользовательским шагом изменения (billing.base = 25, schema.default = 2, billing.unit.size = 100))[align=center;position=under;id=pic_xaas_option_int_with_user_step_free2;number={const(numb_pic_xaas_option_int_with_user_step_free2)}]}
         ![Бесплатная тарифная опция типа integer с пользовательским шагом изменения](./assets/xaas_option_int_with_user_step_free2.png)
         {/caption}
...
1. В секции `billing` задайте параметры ({linkto(#pic_xaas_option_int_with_user_step_prepayed)[text=рисунок %number]}, {linkto(#pic_xaas_option_int_with_user_step_prepayed1)[text=рисунок %number]} и {linkto(#pic_xaas_option_int_with_user_step_prepayed2)[text=рисунок %number]}):
   ...
   * `unit.measurement` — единицы измерения тарифной опции (опционально).
   ...
   ![Платная тарифная опция типа integer c шагом изменени](./assets/xaas_option_int_with_user_step_prepayed.png)
   {/caption}
   ...
   ![Платная тарифная опция типа integer c шагом изменения, значение увеличено на 1 шаг](./assets/xaas_option_int_with_user_step_prepayed1.png)
   {/caption}
   ...
   ![Платная тарифная опция типа integer c шагом изменения, значение увеличено на 2 шага](./assets/xaas_option_int_with_user_step_prepayed2.png)
   {/caption}
...
1. Заполните секцию `schema` таким же образом, как для бесплатной тарифной опции с пользовательским шагом изменения (подробнее — в подразделе {linkto(#int_step_free)[text=%text]}).
   ...
   ![Платная тарифная опция типа integer c шагом изменения](./assets/xaas_option_int_with_user_step_prepayed3.png)
   {/caption}
...
![Тарифная опция-константа типа string](./assets/xaas_option_string_const_example.png){params[width=50%]}
{/caption}
...
1. В секции `schema` задайте параметры:
   ...
   * `type` — тип тарифной опции. Укажите `string`.
   * `default` — значение по умолчанию.
   * Дополнительные параметры, приведённые в подразделе {linkto(../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_configure/xaas_vendor_iboption/xaas_vendor_iboption_schema#IBoption_schema_string)[text=%text]} (опционально).
...
![Тарифная опция типа string с вводом значения](../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_configure/xaas_vendor_iboption/xaas_vendor_iboption_schema/assets/xaas_option_string_input.png){params[width=50%]}
{/caption}
...
![Тарифная опция типа string c выбором значения из списка](./assets/xaas_option_string_enum_example.png){params[width=50%]}
{/caption}
...
![Тарифная опция-константа типа boolean (const = false)](../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_configure/xaas_vendor_iboption/xaas_vendor_iboption_schema/assets/xaas_option_bool_const.png){params[width=50%]}
{/caption}
...
![Тарифная опция-переключатель boolean](../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_configure/xaas_vendor_iboption/xaas_vendor_iboption_schema/assets/xaas_option_bool.png){params[width=50%]}
{/caption}
...
![Платная тарифная опция-переключатель boolean](./assets/xaas_option_bool_paid.png)
{/caption}
...
![Тарифная опция datasource (тип ВМ)](./assets/xaas_option_datasource_flavor.png){params[width=50%]}
{/caption}
...
![Тарифная опция datasource (зона доступности)](./assets/xaas_option_datasource_az.png){params[width=50%]}
{/caption}
...
![Тарифная опция datasource (виртуальная сеть)](./assets/xaas_option_datasource_subnet.png){params[width=50%]}
{/caption}
...
Стоимость диска определяется тарифами {var(sys2)}, поэтому её нельзя задать в описании тарифной опции.
...
![Тарифные опции типа integer и datasource, позволяющие настроить размер и тип диска](./assets/xaas_option_datasource_volume.png){params[width=50%]}
{/caption}