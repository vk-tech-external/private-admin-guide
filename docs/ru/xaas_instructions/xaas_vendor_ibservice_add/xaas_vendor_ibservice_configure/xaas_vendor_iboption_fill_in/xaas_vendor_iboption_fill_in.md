...
1. В секции `schema` задайте параметры:
   ...
   * `type` — тип тарифной опции. Укажите `string`.
   * `default` — значение по умолчанию.
   * Дополнительные параметры, приведенные в подразделе {linkto(../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_configure/xaas_vendor_iboption#IBoption_schema_string)[text=%text]} (опционально).
...
1. В секции `schema` задайте параметры:
   ...
   * `datasource.filter` — фильтры (опционально). Возможные фильтры приведены в подразделе {linkto(../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_configure/xaas_vendor_iboption#IBoption_schema_datasorce)[text=%text]}.
...
1. Опишите тарифную опцию типа `datasource`, получающую данные {var(sys2)} о типах дисков, в файле `parameters/<OPTION_NAME>.yaml`:
   ...
   1. В секции `schema` задайте параметры:
      ...
      * `hint` — описание тарифной опции (опционально).
      * `type` — тип тарифной опции. Укажите `string`.
      * `default` — значение по умолчанию (опционально). Возможные значения приведены в подразделе {linkto(../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_configure/xaas_vendor_iboption#IBoption_schema_datasorce)[text=%text]}.
