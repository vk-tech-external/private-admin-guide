...
1. Получите лог ошибки:
   ...
   * `<CLOUD_HOST>` — доменное имя {var(sys2)}.
   * `<UUID>` — идентификатор инстанса сервиса. Значение соответствует одноименному параметру в запросе на разворачивание инстанса сервиса (подробнее — в разделе {linkto(#deploysystemtest_create)[text=%text]}).
   * `<JWT_TOKEN>` — JWT-токен авторизации, полученный на предыдущем шаге.

   Если при разворачивании инстанса была ошибка, в ответе на команду отобразится ее лог.
...
1. Подключитесь к ВМ, на которой установлен агент, по протоколу удаленного доступа или с помощью VNC-консоли в Портале самообслуживания (подробнее — в документе **Руководство пользователя {var(system)}**).
