...
Перед локальным тестированием убедитесь, что OpenStack PID внесен в список поставщиков (подробнее — в подразделе {linkto(../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_upload/xaas_vendor_ibservice_upload_package#xaas_vendor_ibservice_upload_prepare)[text=%text]}).

При локальном тестировании используйте подсети существующих сетей или создайте новую сеть и используйте ее подсеть (подробнее — в документе **Руководство пользователя {var(system)}** в разделе **Виртуальные сети** → **Сети** → **Создание сетей, подсетей и портов**).

<err>

Использовать общую внешнюю сеть (имя по умолчанию — `external`) запрещено. Возможность использования дополнительных внешних сетей, созданных администратором для конкретных проектов, зависит от конфигурации {var(sys2)} и может быть ограничена.
...
1. Обратитесь к администратору {var(sys2)}, чтобы получить плагин провайдера iVK CS. Сообщите ОС компьютера, на котором будете локально тестировать манифест, и ее архитектуру. Пример: Linux на архитектуре amd64.
...
1. В директории `plugins` создайте следующую структуру директорий:
   ...
   * `<PROVIDER_VERSION>` — версия плагина провайдера. Значение определяется из имени файла плагина. Примеры: для плагина `terraform-provider-vkcs_0.4.2` версия равна `0.4.2`, для плагина `terraform-provider-null_v3.2.1_x5` — `3.2.1`.
