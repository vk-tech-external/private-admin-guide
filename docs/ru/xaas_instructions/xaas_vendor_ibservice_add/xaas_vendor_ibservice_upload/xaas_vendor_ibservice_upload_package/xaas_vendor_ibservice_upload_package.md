...
|Агент (Agent)
|Программное обеспечение, устанавливаемое на ВМ в процессе развертывания сервиса (подробнее — в подразделе {linkto(../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibimage_create/xaas_vendor_ibimage_agent#xaas_vendor_ibimage_agent)[text=%text]})
...
1. Чтобы загрузить архив в Marketplace, выполните запрос к сервису Infra API со следующими параметрами:
   ...
   * `x-service-token`: `<SERVICE_TOKEN>` — сервисный ключ.
      ...
      {caption(Пример запроса на загрузку сервисного пакета в Marketplace для Linux)[align=left;position=above]}
      ```console
      ...
      {caption(Пример запроса на загрузку сервисного пакета в Marketplace для Windows)[align=left;position=above]}
      ```console
      $ curl -v -X POST https://<CLOUD_HOST>/marketplace/api/infra-api/api/v1-public/product ^
      -H "x-service-token: <SERVICE_TOKEN>" ^
      -F "upload=@/home/VKservice.zip"
...
Чтобы проверить, как сервис будет функционировать на {var(sys3)}, после загрузки сервисного пакета протестируйте сервис в тестовом пространстве имен Marketplace:
...
1. Подключите сервис. Убедитесь, что развертывание инстанса сервиса выполнено успешно.
   ...
   Если при развертывании инстанса сервиса не удалось создать ресурс, описанный в манифесте, система развертывания запустит процесс развертывания сервиса повторно. Процесс развертывания инстанса сервиса может занимать до 1,5 ч.
