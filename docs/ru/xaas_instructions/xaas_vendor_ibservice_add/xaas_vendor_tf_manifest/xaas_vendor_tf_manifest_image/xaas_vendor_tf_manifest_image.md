# {appendix-heading(Описание инфраструктуры)[id=xaas_vendor_tf_manifest_image; position=prefix]}
...
1. Создайте ключевую пару ВМ. Используйте ресурс `vkcs_compute_keypair`.
   ...
   resource "vkcs_compute_keypair" "keypair" {
     # Имя ключевой пары, включающее идентификатор развертывания сервиса
...
1. Создайте ВМ из загрузочного образа сервиса. Используйте ресурс `vkcs_compute_instance`.
   ...
   Перед публикацией сервиса ID образа сервиса будет заменен на публичный ID (подробнее — в разделе {linkto(../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_upload/xaas_vendor_ibservice_upload_publish_image#xaas_vendor_ibservice_upload_publish_image)[text=%text]}).
...
## {appendix-heading(Использование типа SDN в ресурсах Terraform)[id=tf_manifest_image_sdn; position=prefix]}
...
Чтобы получить пул внешних IP-адресов, используйте ресурс `vkcs_networking_floatingip` провайдера VK CS. В параметре `pool` задайте имя внешней сети так, чтобы оно определялось автоматически в зависимости от типа SDN. Чтобы правильно указать имена внешних сетей, обратитесь к администратору {var(sys2)}.

<err>

Если в параметре `pool` указать одно фиксированное значение независимо от типа SDN, это приведет к ошибке при развертывании инстанса сервиса в проектах с другим типом SDN.
...
* Если SDN на базе OpenStack Neutron, используется имя внешней сети `ext-net`.
* Если SDN на базе Sprut, используется имя внешней сети `internet`.
