# {appendix-heading(Входные переменные для конфигурации ресурсов)[id=xaas_vendor_tf_manifest_variable; position=prefix]}

Опишите входные переменные для конфигурации ресурсов. Переменные можно условно поделить на группы, приведённые в {linkto(#tab_terraform_variables)[text=таблице %number]}.

{caption(Таблица {counter(table)[id=numb_tab_terraform_variables]} — Переменные Terraform)[align=right;position=above;id=tab_terraform_variables;number={const(numb_tab_terraform_variables)}]}
...
* `instance_uuid` — идентификатор разворачивания сервиса. Используется для описании большинства ресурсов провайдера iVK CS.
