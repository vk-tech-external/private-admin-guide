...
|hosts
|Список имён хостов, где будет выполняться скрипт
...
Тайм-аут задаётся в опции `timeout`
|integer
|`15`
...
Значение задаётся в таком же формате, как и опция `stop_timeout`
...
|once
|Определяет, будет ли скрипт выполняться только один раз или будет выполняться при каждом повторном разворачивании сервиса.

Если задано значение `true`, скрипт будет выполнен только при первоначальном разворачивании сервиса
