...
Для тарифного плана задайте параметры, приведенные в {linkto(#tab_tariff_plan_parameters)[text=таблице %number]}.
...
|metadata
|Определяет тестовые и открытые пространства имен Marketplace, в которых тарифный план будет доступен.

Тестовые пространства имен задаются в ключе `test_ns`.

Открытые пространства имен задаются в ключе `prod_ns`.

Чтобы получить имена пространств имен, обратитесь к администратору {var(sys2)}.

Если пространства имен не заданы, будут использованы значения по умолчанию:
...
Мастер конфигурации тарифного плана, приведенный на {linkto(#pic_xaas_wizard_saas)[text=рисунке %number]}, соответствует следующему содержимому секции `display`:
...
* Параметр `cost` — определяет стоимость плана за месяц `<MONTH_COST>` без учета платных тарифных опций. Задается в валюте страны, где развернут Marketplace. Если план бесплатный, укажите значение `0`.
...
![Тарифный план с бесплатной опцией типа integer с шагом изменения](./assets/xaas_option_int_with_user_step_prepayed4.png){params[printWidth=100%]}
{/caption}
...
![Тарифный план с бесплатной опцией типа integer с шагом изменения, значение опции увеличено на 1 шаг](./assets/xaas_option_int_with_user_step_free_saas.png){params[printWidth=100%]}
{/caption}
...
![Тарифный план с бесплатной опцией типа integer с шагом изменения, значение опции увеличено на 2 шага](./assets/xaas_option_int_with_user_step_free_saas1.png){params[printWidth=100%]}
{/caption}
...
![Тарифный план с предоплатной опцией типа integer с шагом изменения](./assets/xaas_option_int_with_user_step_prepayed4.png){params[printWidth=100%]}
{/caption}
...
![Тарифный план с предоплатной опцией типа integer с шагом изменения, значение опции увеличено на 1 шаг](../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_configure/xaas_vendor_iboption_fill_in/assets/xaas_option_int_with_user_step_prepayed1.png){params[printWidth=100%]}
{/caption}
...
![Тарифный план с предоплатной опцией типа integer с шагом изменения, значение опции увеличено на 2 шага](../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_configure/xaas_vendor_iboption_fill_in/assets/xaas_option_int_with_user_step_prepayed2.png){params[printWidth=100%]}
{/caption}
...
![Тарифный план с предоплатной опцией-переключателем boolean](../../../../xaas_instructions/xaas_vendor_ibservice_add/xaas_vendor_ibservice_configure/xaas_vendor_iboption_fill_in/assets/xaas_option_bool_paid.png){params[printWidth=100%]}
{/caption}

### {appendix-heading(Секция billing с постоплатной числовой тарифной опцией)[id=saas_plan_billing_postpaid; position=prefix]}

Стоимость постоплатной опции типа `integer` или `number` в `<OPTION_BILLING>` описывается параметрами, приведенными в {linkto(#tab_saas_plan_billing_prepaid)[text=таблице %number]}.
...
Стоимость должна соответствовать значению `price`, указанному в методе брокера для получения отчета по фактически использованным ресурсам SaaS-приложения (подробнее — в подразделе {linkto(../../../../xaas_instructions/xaas_vendor_saas_add/xaas_vendor_saas_broker#xaas_vendor_saas_broker)[text=%text]}).
...
![Тарифный план с постоплатной опцией](./assets/xaas_option_postpayed_billing.png){params[printWidth=100%]}
{/caption}
...
Пример описания разных типов опций в формате `JSON` приведен ниже.
      ...
      "parameters": {
        ...
        "properties": {
          ...
          "string_input": {  // Тарифная опция типа string с вводом значения
            ...
            "hint": "электронная почта для выпуска SSL-сертификата"
          },
...
Чтобы тарифную опцию сделать предоплатной, опишите ее стоимость в секции  `plans.billing.options` (подробнее — в подразделе {linkto(#xaas_vendor_saas_plan_billing)[text=%text]}).

</info>

### {appendix-heading(Постоплатные тарифные опции)[id=xaas_vendor_saas_plan_schema_prepaid; position=prefix]}

Постоплатные тарифные опции описываются в секции `properties` параметрами, приведенными в {linkto(#tab_saas_plan_schema_prepaid)[text=таблице %number]}. Стоимость опции описывается в секции {linkto(#xaas_vendor_saas_plan_billing)[text=billing]}.
...
Имя опции в JSON-файле должно соответствовать значению `kind`, указанному в методе брокера, реализующем передачу отчета в Marketplace (подробнее — в подразделе {linkto(../../../../xaas_instructions/xaas_vendor_saas_add/xaas_vendor_saas_broker#xaas_vendor_saas_broker)[text=%text]}).
...
![Постоплатная тарифная опция](./assets/xaas_option_postpayed.png){params[printWidth=100%]}
{/caption}

<info>

Стоимость постоплатной тарифной опции опишите в секции `plans.billing.options` (подробнее — в подразделе {linkto(#saas_plan_billing_postpaid)[text=%text]}).
